class Pathfinder {
	...
	h(from){
		var distance_vector = [this.goal.getPosition()[0]-from[0], this.goal.getPosition()[1]-from[1]];
	...
	A_start(){
		...
		*/
		...
		fScore.set(this.start, this.h(this.start.getPosition()));
		var current = null;
		...
		while(openSet.length > 0){
			...
			for (var i = 0; i < current.getKnownCities().length; i++) {
				var neighbor = current.getKnownCities()[i];
				// tentative_gScore is the distance from start to the neighbor through current
				var tentative_gScore = gScore.get(current) + this.d(current.getPosition(), neighbor.getPosition());
				...
				if (tentative_gScore < gScore.get(neighbor)){
					...
					fScore.set(neighbor, gScore.get(neighbor)+this.h(neighbor.getPosition()));
